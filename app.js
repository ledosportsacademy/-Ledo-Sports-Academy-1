...
// This data will be loaded from MongoDB
let appData = {
  heroSlides: [
    {
      id: 1,
...
function initializeApp() {
  ...
  // Render all content with local data first for fast initial load
  renderHeroSlideshow();
  ...
  try {
    // First, try to fetch all data from the server
    fetchAllData().then(dataLoaded => {
      // If no data was found, initialize the database with default data
      if (dataLoaded) {
      ...
      showMessage('Failed to connect to the database. Using local data instead.', 'error');
    });
  } catch (error) {
    console.error('Error in MongoDB initialization:', error);
    showMessage('Failed to initialize database connection. Using local data instead.', 'error');
  }
